...
func (ssp *streamingServicePlugin) Register(bApp *baseapp.BaseApp, marshaller codec.BinaryCodec, keys map[string]*types.KVStoreKey) error {
	...
	topicPrefix := cast.ToString(ssp.opts.Get(fmt.Sprintf("%s.%s.%s.%s", plugin.PLUGINS_TOML_KEY, plugin.STREAMING_TOML_KEY, PLUGIN_NAME, TOPIC_PREFIX_PARAM)))
	flushTimeoutMs := cast.ToInt(ssp.opts.Get(fmt.Sprintf("%s.%s.%s.%s", plugin.PLUGINS_TOML_KEY, plugin.STREAMING_TOML_KEY, PLUGIN_NAME, FLUSH_TIMEOUT_MS_PARAM)))
	...
	producerConfig := cast.ToStringMap(ssp.opts.Get(fmt.Sprintf("%s.%s.%s.%s", plugin.PLUGINS_TOML_KEY, plugin.STREAMING_TOML_KEY, PLUGIN_NAME, PRODUCER_CONFIG_PARAM)))
	...
	producerConfigKey := fmt.Sprintf("%s.%s.%s.%s",
		plugin.PLUGINS_TOML_KEY, plugin.STREAMING_TOML_KEY, PLUGIN_NAME, PRODUCER_CONFIG_PARAM)
	...
	var err error
	ssp.kss, err = service.NewKafkaStreamingService(bApp.Logger(), producerConfigMap, topicPrefix, flushTimeoutMs, exposeStoreKeys, marshaller, deliverBlockWaitLimit)
	if err != nil {
		return err
	}
