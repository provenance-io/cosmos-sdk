...
type KafkaStreamingService struct {
	...
	deliveredMessages     bool                                            // True if messages were delivered to Kafka, false otherwise.
	deliveredBlockChan    chan struct{}                                   // channel used for signaling the delivery of all messages for the current block to Kafka.
	deliverBlockWaitLimit time.Duration                                   // the time to wait for Kafka service to deliver current block messages before timing out.
}
...
// NewKafkaStreamingService creates a new KafkaStreamingService for the provided producerConfig, topicPrefix, and storeKeys
		...
		for e := range p.Events() {
			...
			case *kafka.Message:
				...
				if err := ev.TopicPartition.Error; err != nil {
					...
					kss.deliveredMessages = false
				} else {
					logger.Debug("Delivered message:", "topic", pTopic, "partition", partition, "offset", offset, "key", key)
					// signal end of block when the `ListenEndBlock` response payload has been delivered.
					// There is only one EndBlock Response per block so this is safe.
...
func (kss *KafkaStreamingService) ListenBeginBlock(
	...
	if err := kss.writeAsJsonToKafka(string(BeginBlockReqTopic), key, &req); err != nil {
		return err
	}

	// write state changes
	if err := kss.writeStateChange(string(BeginBlockEvent), eventId); err != nil {
		return err
	}

	// write res
	key = fmt.Sprintf(MsgKeyFtm, kss.currentBlockNumber, EndBlockEvent, 1, ResponseEventType, 1)
	if err := kss.writeAsJsonToKafka(BeginBlockResTopic, key, &res); err != nil {
		return err
	}
...
func (kss *KafkaStreamingService) setBeginBlock(req abci.RequestBeginBlock) {
	...
	kss.deliveredMessages = true // Reset to true. Will be set to false when delivery of any message fails.
}
...
func (kss *KafkaStreamingService) ListenDeliverTx(
	...
	if err := kss.writeAsJsonToKafka(DeliverTxReqTopic, key, &req); err != nil {
		return err
	}

	// write state changes
	if err := kss.writeStateChange(DeliverTxEvent, eventId); err != nil {
		return err
	}

	// write res
	key = fmt.Sprintf(MsgKeyFtm, kss.currentBlockNumber, DeliverTxEvent, eventId, ResponseEventType, 1)
	if err := kss.writeAsJsonToKafka(DeliverTxResTopic, key, &res); err != nil {
		return err
	}
...
func (kss *KafkaStreamingService) ListenEndBlock(
	...
	if err := kss.writeAsJsonToKafka(EndBlockReqTopic, key, &req); err != nil {
		return err
	}

	// write state changes
	if err := kss.writeStateChange(EndBlockEvent, eventId); err != nil {
		return err
	}

	// write res
	key = fmt.Sprintf(MsgKeyFtm, kss.currentBlockNumber, EndBlockEvent, eventId, ResponseEventType, eventTypeId)
	if err := kss.writeAsJsonToKafka(EndBlockResTopic, key, &res); err != nil {
		return err
	}
...
func (kss *KafkaStreamingService) ListenSuccess() <-chan bool {
	// Synchronize the work between app.Commit() and Kafka.
	// Wait unit Kafka has acknowledged delivery of messages for the current processed block
	// or until the timeout is reached before responding back to the app.Commit() call.
	var deliveredBlock bool
	maxWait := time.NewTicker(kss.deliverBlockWaitLimit)
	defer maxWait.Stop()
	loop:
		for {
			// No reason to wait for block data to finish writing
			// if any of the block's messages failed to be delivered
			if kss.deliveredMessages == false {
	...
	kss.successChan <- kss.deliveredMessages
	return kss.successChan
}
...
func (kss *KafkaStreamingService) writeStateChange(event string, eventId int64) error {
	...
	for i, stateChange := range kss.stateCache {
		...
		if err := kss.writeAsJsonToKafka(StateChangeTopic, key, kvPair); err != nil {
			return err
		}
	}
...
func (kss *KafkaStreamingService) writeAsJsonToKafka(topic string, key string, data proto.Message) error {
	kodec := kss.codec.(*codec.ProtoCodec)
