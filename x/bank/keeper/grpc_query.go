...
func (k BaseKeeper) QuarantinedFunds(goCtx context.Context, req *types.QueryQuarantinedFundsRequest) (*types.QueryQuarantinedFundsResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "empty request")
	}
	...
	resp := &types.QueryQuarantinedFundsResponse{}
	...
	if len(toAddr) > 0 && len(fromAddr) > 0 {
		...
		resp.Pagination, err = query.FilteredPaginate(
			...
			func(key, value []byte, accumulate bool) (bool, error) {
				...
				if accumulate {
					kToAddr, kFromAddr := types.ParseQuarantineRecordKey(key)
