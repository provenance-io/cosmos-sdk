...
func TestCountAuthorization(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			var authorization *CountAuthorization
			testNew := func() {
				authorization = NewCountAuthorization(tc.msgType, tc.count)
			}
			...
			var actAcceptResp AcceptResponse
			...
			if assert.NotPanics(t, testAccept, "Accept") {
				...
				case assert.NotNil(t, actAcceptResp.Updated, "AcceptResponse.Updated"):
					actAuth, isCount := actAcceptResp.Updated.(*CountAuthorization)
